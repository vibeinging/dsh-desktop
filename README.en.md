...
Official Web Sessions receive native window Host access only for allowlisted methods and never carry dsh-work user or project identity. `smoke:native-host` has now exercised state, focus, minimize, maximize, and restore through the packaged macOS arm64 Electron app, then removed the test Bundle through the official Profile command. File/directory dialogs and cross-platform window interaction still require release-environment acceptance.
