...
| [DSH @file](https://github.com/omdsh-dev/dsh-at-file) | Search and reference workspace files from the input box | The official `inputTriggers` and input overlay are connected; use the independent product-reference Bundle for App-authorized roots first, then test the community package after it pins rc.6 |
...
| [DSH Toolkit](https://github.com/omdsh-dev/dsh-toolkit) | Deterministic tools for time, encoding, JSON, CSV, diffs, and statistics | Fixed-commit Host preflight completed; two DSH SDK ranges do not match the exact rc.6 release line, so adoption waits for the community migration |
...
DeepSeek Harness Desktop App is a DSH Profile distribution and an Electron plugin host. It does not disguise windows, updates, preload, or system permissions as ordinary plugins that can be installed into the official Web. Reusable functionality is split into three levels: `portable` Bundles can be installed into both the official Web and the desktop app; `desktop-adapter` Bundles use the DSH lifecycle but depend on explicit desktop Host contracts; `desktop-shell` is responsible only for windows, the root layout, and security isolation. The theme package and `dsh-model-inheritance` are already `portable`; the latter uses only the official Agent lifecycle so sub-Agents inherit their parent Agent's resolved model target. `dsh-work-product-host-ipc`, `dsh-client-product-commands`, `dsh-client-product-references`, `dsh-client-product-search-mode`, `dsh-client-product-attachments`, the project tools, the Canvas tools, the structured UI tools, the workbench pages, the product bridge, and the Office tools package are `desktop-adapter`; and `dsh-work-shell` is a `desktop-shell`. Parent-process transport is now owned solely by `dsh-work-product-host-ipc`; the project tools, Canvas tools, structured UI tools, and product bridge consume `productHost`, the Office tools consume the narrower `officeArtifactHost`, and feature Bundles no longer access IPC directly. `dsh-client-product-commands` independently owns the official input source, bilingual catalog, and Session token consumption for `/new`, `/runs`, and `/trace`; the product shell exposes only the narrow `dshWorkProductActions` service, so these desktop actions can be installed, disabled, or replaced independently through the Profile. `dsh-client-product-references` registers file and conversation sources through the same official `inputTriggers` service and consumes only the Session-fenced `dshWorkProductReferences` catalog published by the product shell; the formal Client no longer starts the App-local `@` or `#` scanner. `dsh-client-product-search-mode` contributes the web policy control through the standard `conversation.input.left` seat and consumes only the `dshWorkProductSearchMode` service published by the product shell; the formal Client renders only this control, while standalone development keeps the local fallback. The policy enters the App protocol with the next prompt and is not an official Session projection, so the Bundle is explicitly a `desktop-adapter`. The product bridge no longer registers any Tool, workbench page, model inheritance, or memory and retains only App and project instruction context; long-term memory now uses the community `dsh-native-memory` Bundle installed through the official Profile. The official Tool UI plugin now enters the product conversation through `conversation.chat.node`, and community Client plugins can replace individual Tool views through the nested `tool.call.toolview` seat; the built-in product Tool card appears only when no plugin takes over. After Inspect is clicked on a Tool row, the product right column keeps only the Session and call selection while the official or community `conversation.details.tool` plugin renders terminal, read, search, web, diff, and generic results. The independent rc.6 Tool plugin still reads a small locale subset registered by the official Conversation page shell. Because the product must disable that duplicate page shell, `dsh-work-shell` registers only the zh/en keys used by Tool UI through the same Locale service and releases them with its lifecycle; it does not copy Conversation state or the rest of the page dictionary. The product shell also follows the official `IConversation` contract to publish the `conversation` service and delegates directly to the DSH Session, allowing the Agent Preset plugin to appear in the `conversation.hero.agentPreset` seat and stage the choice for the next Session. In the formal Client, the product Queue and compact Plan and permission readouts subscribe directly to the same Client Session's `ConversationSnapshot.queue` and `session.projections.faceOf(...)`; the App protocol stream only resolves the product conversation to its DSH Session and continues carrying message content that has not moved to official Chat, so it is no longer a second state authority. The Plugin Center uses the same host capability list to separate fully supported, partially supported, and unmapped seats; `conversation.chat.node` currently promises only `tool-call`, so other message nodes are not reported as available. `dsh-workbench-pages` independently contributes the Review, Browser, Files, Artifacts, and Sites page catalog. These pages will use community plugins first, with missing pieces then split into independent Bundles one by one. OpenPencil remains the community design plugin option instead of duplicating the existing versioned Canvas implementation. Product components will no longer be added to the shell.
...
`dsh-client-product-attachments` owns the formal Client's draft-attachment presentation: it enters the explicit desktop-shell root extension `dsh-work.composer.pre-session` before the first DSH Session exists, then moves to the standard `conversation.input.dock` after a Session is bound. It consumes only the Session-fenced `dshWorkProductAttachments` service, so the plugin never sees local paths or attachment bytes. Its image rail directly reuses the official rc.6 `AttachmentRail` React atom; the desktop adapter renders files, folders, media, and document selections that the official atom does not yet support. The official Web `ui-conversation` keeps browser `File` objects and temporary image ids inside its private controller, while the desktop App admits local paths through its trusted Host, so this Bundle remains an explicit `desktop-adapter` instead of pretending that the two draft identities are one protocol.

The formal Client uses the official rc.6 `dsh-client-ui-permission-presets` for the entire permission surface: the official `/permission` popup changes the current Session, the official Settings row changes the new-session default, and the plugin owns the Full access risk gate. The App has deleted its own `PermissionPicker`, and the product bridge no longer subscribes to or copies the `permissions` projection.
