...
社区插件不是 DeepSeek 或 DSH 官方产品。发行包只内置经过逐包审查的任务看板，不携带 `dsh-web-ui` 聚合包、社区插件管理器或未审查的皮肤资产。任务看板的 `package.json` 声明 Apache-2.0，但 npm tarball 内 `LICENSE` 是带作者署名的 BSD-3-Clause；发行包按 tarball 内原文保留 BSD-3-Clause 许可证和署名，并在生成的公告中记录这项元数据差异。其他候选必须通过固定来源、依赖、权限、许可证、真实 Electron 启动和卸载回归。没有可核对再分发许可的皮肤、插图、字体、音频和视频不得进入安装包。
