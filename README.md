...
- 安装包：`package:mac:dir` 生成随包目录，`smoke:official-web` 验证随包官方 Web。Browser Workspace 截图先使用 Electron `capturePage`，在当前 Viz 不可用时回退到受控的 DevTools Page 截图协议；签名、公证、Windows 实机、干净用户更新和卸载回归仍需在对应发行环境完成。
