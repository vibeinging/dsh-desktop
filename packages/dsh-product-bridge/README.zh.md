...
- 已发布的 rc.6 SDK 不包含 ProductHost 包。因此目前由私有的 `@deepseek-ai/dsh-work-product-host-ipc` 适配器提供与传输无关的服务；这个 Bundle 只持有应用指令上下文。模型继承和记忆是独立的 portable/社区 Bundle，而项目、Canvas、Structured UI 和 Office 工具是独立的桌面消费方。它可以在将来切换到 DSH 发布的提供方，无需把 IPC 移回功能代码。
