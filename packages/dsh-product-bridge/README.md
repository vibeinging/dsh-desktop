...
This private Profile Bundle extends the official DSH Web Profile without importing or changing a DSH source checkout. It owns only the App instruction context and no longer registers any Tool, workbench page, model-inheritance hook, or memory provider. It consumes the session-addressed `productHost` service provided by `@deepseek-ai/dsh-work-product-host-ipc`; the portable `@deepseek-ai/dsh-model-inheritance` Bundle owns parent-to-sub-Agent model inheritance, the separate `@deepseek-ai/dsh-project-tools` Bundle owns `project_list` and `conversation_list`, `@deepseek-ai/dsh-canvas-tools` owns the four Canvas/Site tools, `@deepseek-ai/dsh-structured-ui-tools` owns `ui_render`, `@deepseek-ai/dsh-office-tools` consumes `officeArtifactHost`, and `@deepseek-ai/dsh-workbench-pages` owns the app workbench catalog. Long-term memory comes from the reviewed community `dsh-native-memory` Profile Bundle instead of an App-specific database and injection path.
