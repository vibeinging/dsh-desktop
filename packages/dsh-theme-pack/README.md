...
The Bundle owns installation and provenance through `dsh.bundle.patch`, and its patch inserts the `dsh-theme-pack` Cordis plugin row so Profile also owns runtime lifecycle. Its standard `dsh.client` entry applies the blue palette through DSH `theme.overrideTokens()`, so the same package works in the official Web Profile and in DeepSeek Harness Desktop App. The transitional `dshWork.themes` descriptor still supplies the desktop theme picker until that product surface moves to the standard DSH theme registry; it contains only renderer-safe color and appearance tokens and never injects raw CSS.
