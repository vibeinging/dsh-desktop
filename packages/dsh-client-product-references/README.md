...
This private desktop-adapter Bundle contributes product workspace files and App conversations through the official `inputTriggers` registry. Both sources use the rc.6 `@` trigger alongside the official subagent source. Picking a file writes `@<authorized-path> ` into the shared draft, while picking a conversation writes `#<title> `; the package owns the source roster and matching but never reads the product DOM or App storage.
