...
在本次精选清单组合字段、基础服务管理边界和打包 smoke 入口提交后，当前 `dev` 工作区重新运行 `npm run test:release` 为 138 项中 136 项通过、2 项按条件跳过、0 项失败；`npm run typecheck`、`npm run check:release-artifacts` 和 `npm run release:check:static` 也通过，当前静态门禁为 14/14。此结果是当前源码提交的回归证据，不替代干净 worktree、真实 Windows 或 Apple 公证验收。
