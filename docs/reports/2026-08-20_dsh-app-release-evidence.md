...
随后在同一打包目录上运行 `npm run smoke:native-host:dialogs` 对应的 `--dialogs` 模式，使用本次运行生成的测试文件和目录完成两次 macOS 原生面板选择。结果回执 `/tmp/dsh-native-host-dialogs-final.json` 的 `status` 为 `passed`、`evidence_level` 为 `packaged-electron-native-host-dialogs`，检查包括官方 Profile 安装、官方 Web Tool、session-bound 文件选择、session-bound 目录选择和官方 Profile 卸载；截图已归档到 `electron/.desktop-build/evidence/native-host-dialogs/`。该证据覆盖 macOS arm64 文件/目录对话框，不替代跨平台窗口或其他架构验收。
