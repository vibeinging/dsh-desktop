...
本地工作区位于 `dev`，本批基线提交为 `97303f9`（`ci: require bound release evidence`），主工作区仍有其他窗口的未提交改动，包括 legacy Renderer 和多个评估文件。本批只修改发行回执/门禁脚本、macOS smoke、回执测试、精选插件兼容测试和本报告，没有 reset、stash、clean 或覆盖 Renderer 脏文件。
...
- 四类回执现在有固定 `evidence_level` 和必需 checks；字符串数组、`passed:false`、缺项、重复项、未知项和错误 level 均被拒绝。正式 gate 按 kind 调用专用 validator，并验证 CI commit 等于当前 checkout，签名身份只取实际 App 签名结果。
...
- `release:verify:mac -- --require-evidence` 现在读取并验证四类真实回执，缺 key、缺回执、错误 commit、错误产物 hash、错误签名身份或缺当前 manifest 都阻断。普通静态检查仍不要求真实密钥和真实回执；DMG 安装器在成功 detach 后才原子写入回执，detach 失败会删除旧回执并失败。
...
| Profile 集成测量 | `npm run measure:featured-plugins` | 7/7 逐包 cold Web 测量通过：404、415、409、414、412、427、408 ms | 通过 |
