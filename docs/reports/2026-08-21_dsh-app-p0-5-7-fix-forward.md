...
本地工作区位于 `dev`，本轮修复从 `de6c9d3`（`docs: make featured measurement report authoritative`）开始，主工作区仍有其他窗口的未提交改动，包括 legacy Renderer 和多个评估文件。本轮只修改发行回执/门禁脚本、macOS native Host smoke、回执测试、macOS evidence workflow 和本报告，没有 reset、stash、clean 或覆盖 Renderer 脏文件。
...
| 单元/回归 | `node --import ./eval/tests/setup.mjs --test --test-concurrency=1 eval/tests/release-safety.test.mjs` | 20 tests，20 pass，0 fail，0 skipped；含 native Host 两种 mode 的合法/最小/错 mode/缺操作负例、四类回执负例、live-model 错误历史和 commit 绑定负例 | 通过 |
