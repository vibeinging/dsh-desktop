...
本地工作区位于 `dev`，基线提交为 `29a113e`（`fix: restore official Web plugin lifecycle`），主工作区仍有其他窗口的未提交改动，包括 legacy Renderer 和多个评估文件。本批只修改发行脚本、macOS workflow、回执测试、精选插件兼容测试和本报告，没有 reset、stash、clean 或覆盖 Renderer 脏文件。
...
- `release:verify:mac -- --require-evidence` 现在读取并验证四类真实回执，缺 key、缺回执、错误 commit、错误产物 hash、错误签名身份或缺当前 manifest 都阻断。普通静态检查仍不要求真实密钥和真实回执。
...
| 单元/回归 | `node --import ./eval/tests/setup.mjs --test --test-concurrency=1 eval/tests/release-safety.test.mjs` | 15 tests，15 pass，0 fail，0 skipped | 通过 |
| 精选产物 | `node scripts/generate-featured-plugin-artifacts.mjs` | 7 个 tarball 生成 | 通过 |
| Profile 集成测量 | `npm run measure:featured-plugins` | 7/7 逐包 cold Web 测量通过：834、418、409、417、439、502、441 ms | 通过 |
| 静态发行门禁 | `npm run release:check:mac:static` | ready=true，13 pass，0 block，0 manual | 通过 |
...
测量报告同时记录生成时 checkout 的 `git_commit_sha`；最终门禁以生成文件中的该字段为准，代码或文档提交后必须重新测量。精选源 manifest SHA-256 为 `a6daddac851618bc00794b25ef0d901fc69bef85c342ddc41c4cdac165c8f636`，随包 manifest SHA-256 为 `32e6a0ffc5a5f56f89e6ce7e994b2f5ecc12ca0b69fb4f81d27bc1693b77d09a`。
...
- 仍需在最终代码收敛后重新构建、签名、公证 App/DMG，运行 macOS 安装器、native Host 和真实 live-model smoke，验证四类回执并上传 CI artifact。
- Windows、原生 Intel x64 和真实 DeepSeek live-model 仍属于外部环境证据；Rosetta、静态源码检查或 loopback smoke 不能替代它们。
